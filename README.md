...
Last updated 2025-12-10 to work with mdbook v0.5.0

This repo contains a template that I use when creating new "books" using [mdbook](https://rust-lang.github.io/mdBook/), so I don't have to manually copy and edit a bunch of files. It contains an almost-empty "book" generated by mdbook, with the following customizations:

* Section lines, which are horizontal bars of varying thickness above `H1`, `H2`, and `H3` headers. (See the [generated output](https://kg4zow.github.io/mdbook-template/) for an example of what these look like.)
...
* Automatic git commit and version information at the bottom of the navigation panel on the left, and/or at the bottom of each generated page.
...
* **`book.toml`** - tells `mdbook` how to generate the book. The following line tells `mdbook`, when it generates HTML output, to include the following CSS files in addition to the ones included in the theme.
    ...
    This example has a second file in the `additional-css` list. This other file is used by the Version/Commit feature, which is explained below.
...
This is not something that `mdbook` does on its own, I had to figure out how to make it happen, however I was not the first person to figure this out. I was able to get this working based on information from the links in [this GitHub issue](https://github.com/rust-lang/mdBook/issues/494). My contributions, if any, are...

* Bundling those commands into a `Makefile`, in a template repo that I use when starting new books, and that others are free to use.

* Figuring out how and where to edit the "theme" files to put the version info at the bottom of the page and/or the Table of Contents, and then later figuring out a way to copy and customize the files from `mdbook`'s source code.
...
The items describe below are involved in making this work.


### The `theme-template/` Directory

This directory contains the `mdbook-fix-templates` script, which builds customised template files containing placeholders where the commit information will be inserted. The customizations come from HTML fragment files which are in the same directory.

When `mdbook` renders the book (converts the Markdown source files into HTML), it uses these templates to format the final HTML files.

One of the HTML fragments can be used to add text *above* the Table of Contents. I use this at work to add a link back to a central "list of books" page, which has links to all of the books I maintain at work.

This is documented in the [README.md file](theme-template/README.md) in the `theme-template/` directory.

### The `version-commit` Script

This is a shell script which is executed every time `mdbook` renders the book (i.e. converts the files from Markdown to HTML). It does two things:

1. **Reads a stream of data provided by `mdbook`, and prints part of it out.**

    The script is what `mdbook` calls a "[preprocessor](https://rust-lang.github.io/mdBook/for_developers/preprocessors.html)". When running a preprocessor, `mdbook` sends it a JSON list with two elements. The first element contains metadata about the overall "book", and the second element contains the book's content. This includes a JSON representation of the contents of the input Markdown files.

    Preprocessors are expected to read this JSON stream and output a *possibly modified* copy of *just the book's content* (i.e. just the second element of the original JSON). If the script modifies the JSON, this changes the "input" that `mdbook` converts to HTML, which modifies the generated HTML pages.

    In our case, we aren't modifying anything about the Markdown input itself. The only reason we're writing a "filter" is because it's the only mechanism `mdbook` provides to run user-supplied scripts before building the HTML pages. Because we aren't modifying the input, the script just prints the second element as-is.

2. **Reads the necessary files from the "`theme-template/`" directory, substitutes a few values into them, and writes the modified files to the "`theme/`" directory.**

    Specifically, the script substitutes values for the following tags:

    * **`@VERSION_COMMIT_HASH@`**

        Replaced with the git commit hash information.

        The script runs "`git describe --always --tags --dirty`" to get this information. This includes the most recent tag and how many commits "ahead" of that tag we are, the commit hash (if it's different from the tag), and whether or not the content is "dirty" (i.e. if the working directory contains content which hasn't been committed yet).

    * **`@VERSION_COMMIT_TIME@`**

        Replaced with the timestamp of that commit.

        The timestamp is UTC time, in the "ISO 8601-like format" used by the [`git log --date=iso`](https://git-scm.com/docs/git-log#Documentation/git-log.txt---dateformat) option, which looks like this:

        `2025-12-13 01:09:10 +0000`

    * **`@VERSION_COMMIT_MDBOOK@`**

        Replaced with the version of `mdbook` being used.

        The script runs "`mdbook --version`" to get this information.

    * **`@VERSION_COMMIT_NOW@`**

        Replaced with the timestamp when the script was executed.

        This timestamp is also UTC time, using the same format described above.

    Any other files in the "`theme-template/`" directory are ignored.

When generating the HTML files, `mdbook` uses the files in the "`theme/`" directory *instead of* the built-in theme files with the same names. Adding the commit information in the theme files is what makes that information show up in the final HTML pages.


### `version-commit.css`

This file contains CSS to control the formatting of the items being added to the template. It contains five CSS selectors, matching the `class=` attributes used in the HTML fragments.

In my case I wanted the text of the Version/Commit information to be a bit smaller than the normal text in the Table of Contents, so [the file I use](./version-commit.css) contains some simple directives to set the text size and line spacing. I like the way it looks, obviously you're free to customize it any way you like.


### Customizing the Results

* If you want to change how the "version" or the timestamps are presented, e.g. to use a different "`git describe`" command to get the commit info, or to format the timestamps differently, edit the `version-commit` script.

* If you want to change the blocks being added to the templates, e.g. to show *only* the commit information but not the timestamp when it was generated, edit the `theme-template/*.html` files.

* If you want to change *where* the blocks are added within the pages, edit the `theme-template/mdbook-fix-templates` script and then run it again.

* If you want to remove one of the added blocks entirely, e.g. to *not* show the commit information at the bottom of every page but still show it at the bottom of the Table of Contents, make the appropriate `theme-template/*.html` file empty. The `mdbook-fix-template` script will still insert the file's contents into the appropriate template files, but it will be inserting *nothing*, and the output will not include anything new where that added block would normally be inserted.

    An easy way to make a file empty is to run "`> filename`", which redirects the output from not running any command at all, into `filename`.

* **If you don't want the commit information to be added at all**, remove the `[preprocessor.version-commit]` section from the `book.toml` file. If the commit information *has been* added, you will also need to remove the customized `*.hbs` files from the `theme/` directory.


## `book.toml`

This file is what tells `mdbook` how to create the HTML files for the book. This repo adds two sections to the default file created by the `mdbook init` command.
...
* **The `additional-css` item in the `[output.html]` section** tells `mdbook` to include one or more additional CSS files in the generated HTML files.
    ...
    This example has two CSS files: `section-lines.css` *is* the Section Lines feature (explained above), and `version-commit.css` is used to format the text added by the Version Commit feature.
...
Some files in this repo's `/theme-template/` directory may have been copied from [the mdbook source code](https://github.com/rust-lang/mdBook/) and then modified. Any such files are technically covered by the Mozilla Public License 2.0, [as noted in their repo](https://github.com/rust-lang/mdBook/blob/master/LICENSE).
