...
> &#x2139;&#xFE0F; [`https://mdbook-template.jms1.info/`](https://mdbook-template.jms1.info/) is a copy of the HTML generated by this template, in case you want to see what it looks like.
...
If the generated HTML files are going to be hosted on a web server somewhere ...

* Edit `Makefile`
* Update the command(s) for the `push:` target so they upload the `book/` directory to wherever they need to go.

If your text editor creates "temp files" which need to *not* be added to the repo ...
...
### Create a GitHub repo and push the new book to it
